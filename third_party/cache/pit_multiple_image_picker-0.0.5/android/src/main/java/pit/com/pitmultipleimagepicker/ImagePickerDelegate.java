...
public class ImagePickerDelegate
    ...
    @VisibleForTesting
    static final int REQUEST_CODE_CHOOSE_IMAGES_FROM_GALLERY = 12341;
    @VisibleForTesting
    ...
    public void chooseImageFromGallery(MethodCall methodCall, MethodChannel.Result result) {
        Log.d("ricric", "ngantuk 1");
        if (!setPendingMethodCallAndResult(methodCall, result)) {
            Log.d("ricric", "ngantuk 2");
            finishWithAlreadyActiveError();
            return;
        }

        if (!permissionManager.isPermissionGranted(Manifest.permission.READ_EXTERNAL_STORAGE)) {
            Log.d("ricric", "ngantuk 3");
            permissionManager.askForPermission(
                    Manifest.permission.READ_EXTERNAL_STORAGE, REQUEST_EXTERNAL_IMAGE_STORAGE_PERMISSION);
            return;
        }
        Log.d("ricric", "ngantuk 4");
    ...
    private void launchPickImagesFromGalleryIntent() {
        int maxImages = methodCall.argument("maxImages");
        Intent intent = new Intent(activity, CustomPhotoGalleryActivity.class);
        intent.putExtra("maxImages", maxImages);
        activity.startActivityForResult(intent, REQUEST_CODE_CHOOSE_IMAGES_FROM_GALLERY);
    }
        ...
        switch (requestCode) {
            case REQUEST_EXTERNAL_IMAGE_STORAGE_PERMISSION:
                if (permissionGranted) {
                    if (methodCall.argument("maxImages").equals(1)) {
                        launchPickImageFromGalleryIntent();
                    } else {
                        launchPickImagesFromGalleryIntent();
                    }
    ...
    private void handleChooseImagesResult(int resultCode, Intent data) {
        if (resultCode == Activity.RESULT_OK && data != null) {
            ...
            for (String anImagesPath : imagesPath) {
                Log.d("ricric", "imagePath = " + anImagesPath);
            }
            handleImagesResult(new ArrayList<>(Arrays.asList(imagesPath)));
            return;
        }
    ...
    private void handleImagesResult(List<String> paths) {
        if (pendingResult != null) {
            ...
            for (String path : paths) {
                ...
                finalImagePaths.add(finalImagePath);
            }
            finishWithSuccessMultiple(finalImagePaths);
        } else {
    ...
    private void finishWithSuccessMultiple(List<String> imagePath) {
        pendingResult.success(imagePath);
        clearMethodCallAndResult();
    }
