#import "BaseWebviewPlugin.h"

static NSString *const CHANNEL_NAME = @"base_webview";

// UIWebViewDelegate
@interface BaseWebviewPlugin() <WKNavigationDelegate, UIScrollViewDelegate> {
    BOOL _enableAppScheme;
    BOOL _enableZoom;
    
}

@property (nonatomic, strong) NSArray *overridableUrlsArray;
@property (nonatomic, strong) NSArray *schemesArray;

@end

@implementation BaseWebviewPlugin
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    ...
    BaseWebviewPlugin* instance = [[BaseWebviewPlugin alloc] initWithViewController:viewController];
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    if ([@"launch" isEqualToString:call.method]) {
        ...
        [self stopLoading];
        result(nil);
    } else if ([@"setOverridableUrls" isEqualToString:call.method]) {
        [self setOverridableUrls: call];
    } else if ([@"setUriSchemes" isEqualToString:call.method]) {
        [self setSchemes:call];
    } else {
...
- (void)navigate:(FlutterMethodCall*)call {
    if (self.webview != nil) {
            ...
            if ( [withLocalUrl boolValue]) {
                ...
                if (!htmlString && [htmlString isKindOfClass:[NSString class]] && htmlString.length > 0) {
                    NSURL *baseUrl = (url != nil && url.length > 3 && [htmlString rangeOfString:@"http"].location != NSNotFound) ? [NSURL URLWithString:url] : nil;
...
- (void)setOverridableUrls:(FlutterMethodCall*)call {
    NSArray *urls = call.arguments[@"overridableUrls"];
    self.overridableUrlsArray = urls;
}

- (void)setSchemes:(FlutterMethodCall*)call {
    NSArray *schemes = call.arguments[@"schemes"];
    self.schemesArray = schemes;
}
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction
    ...
    if (_schemesArray && _schemesArray.count > 0) {
        for (NSString *scheme in _schemesArray) {
            if (scheme && scheme.length > 0 && [webView.URL.scheme isEqualToString:scheme]) {
                id data = @{@"url": navigationAction.request.URL.absoluteString};
                [channel invokeMethod:@"onSchemeMatched" arguments:data];
    ...
    if (_overridableUrlsArray && _overridableUrlsArray.count > 0) {
        for (NSString *oUrl in _overridableUrlsArray) {
            if (oUrl && urlString.length > oUrl.length && [urlString rangeOfString:oUrl].location != NSNotFound) {
                id data = @{@"url": navigationAction.request.URL.absoluteString};
                [channel invokeMethod:@"onUrlOverridden" arguments:data];
                decisionHandler(WKNavigationActionPolicyCancel);
                return;
            }
        }
