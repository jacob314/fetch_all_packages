 ...
 * BaseWebviewPlugin
 */
public class BaseWebviewPlugin implements MethodCallHandler, PluginRegistry.ActivityResultListener {
  private Activity activity;
  private WebviewManager webViewManager;
  static MethodChannel channel;
  private static final String CHANNEL_NAME = "base_webview";

  public static void registerWith(PluginRegistry.Registrar registrar) {
    channel = new MethodChannel(registrar.messenger(), CHANNEL_NAME);
    final BaseWebviewPlugin instance = new BaseWebviewPlugin(registrar.activity());
    registrar.addActivityResultListener(instance);
    channel.setMethodCallHandler(instance);
  }

  private BaseWebviewPlugin(Activity activity) {
  ...
  public void onMethodCall(MethodCall call, MethodChannel.Result result) {
    switch (call.method) {
      ...
      case "stopLoading":
        ...
        break;
      case "setOverridableUrls":
        Log.d(CHANNEL_NAME, "setOverridableUrls");
        setOverridableUrls(call, result);
        break;
      case "setUriSchemes":
        Log.d(CHANNEL_NAME, "setUriSchemes");
        setUriSchemes(call, result);
        break;
      default:
  ...
  private void openUrl(MethodCall call, MethodChannel.Result result) {
    ...
    boolean withWideViewPort = call.argument("withWideViewPort");
    boolean autoLoadImage = call.argument("autoLoadImage");
    boolean allowFileAccess = call.argument("allowFileAccess");
    boolean appCacheEnabled = call.argument("appCacheEnabled");
    String defaultTextEncodingName = call.argument("defaultTextEncodingName");
    Long cacheMaxSize = call.argument("cacheMaxSize");
  ...
  private void setOverridableUrls(MethodCall call, MethodChannel.Result result) {
    List<String> overridableUrls = call.argument("overridableUrls");
    if (overridableUrls != null) {
      webViewManager.setOverridableUrls(overridableUrls);
    }
  }

  private void setUriSchemes(MethodCall call, MethodChannel.Result result) {
    List<String> uriSchemes = call.argument("schemes");
    if (uriSchemes != null) {
      webViewManager.setUriSchemes(uriSchemes);
    }
  }
