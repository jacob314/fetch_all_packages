...
public class FlutterMobPlugin implements MethodCallHandler {
    ...
    public void onMethodCall(MethodCall call, Result result) {
        ...
        switch (call.method) {
            ...
            case "submitCode":
                ...
                break;
            case "config":
                HashMap<String, Object> wechat = call.argument("wechat");
                if (wechat != null) {
                    ShareSDK.setPlatformDevInfo(Wechat.NAME, wechat);
                }
                HashMap<String, Object> wechatMoments = call.argument("wechatMoments");
                if (wechat != null) {
                    ShareSDK.setPlatformDevInfo(WechatMoments.NAME, wechatMoments);
                }
                HashMap<String, Object> qq = call.argument("qq");
                if (wechat != null) {
                    ShareSDK.setPlatformDevInfo(QQ.NAME, qq);
                }
                HashMap<String, Object> sina = call.argument("sina");
                if (wechat != null) {
                    ShareSDK.setPlatformDevInfo(SinaWeibo.NAME, sina);
                }
                result.success(null);
                break;
            case "qqLogin":
            ...
            case "wechatLogin":
                ...
                wechatPlatform.setPlatformActionListener(listener);
                wechatPlatform.authorize();
                break;
            case "sinaLogin":
                listener.setResult(result);
                Platform sinaPlatform = ShareSDK.getPlatform(SinaWeibo.NAME);
                sinaPlatform.setPlatformActionListener(listener);
    ...
    private class Listener implements PlatformActionListener {
        ...
        public void onComplete(Platform platform, int i, HashMap<String, Object> hashMap) {
            HashMap<String, Object> map = new HashMap<>();
            map.put("status", 0);
            map.put("msg", platform.getDb().exportData());
            result.success(map);
        }
