...
public final class BillingPlugin implements MethodCallHandler {
    ...
    public void onMethodCall(MethodCall methodCall, Result result) {
        ...
        } else if ("consumeProduct".equals(methodCall.method)) {
            consumeProduct(methodCall.<String>argument("purchaseToken"), result);
        } else {
            result.notImplemented();
        }
    }

    private void consumeProduct(final String purchaseToken, final Result result) {
                ...
                billingClient.consumeAsync(purchaseToken, new ConsumeResponseListener() {
                    ...
                    public void onConsumeResponse(int responseCode, String purchaseToken) {
                        if (responseCode == BillingResponse.OK) {
                            ...
                            result.error("ERROR", "Failed to consume product!", null);
                        }
                    }
                });
            }
    ...
    private List<String> getIdentifiers(List<Purchase> purchases) {
        ...
        for (Purchase purchase : purchases) {
            identifiers.add(purchase.getSku() + ":-:" + purchase.getPurchaseToken());
        }
        ...
        public void onPurchasesUpdated(int resultCode, List<Purchase> purchases) {
            if (resultCode == BillingResponse.OK && purchases != null) {
                ...
                for (String identifier : identifiers) {
                    final Result result = pendingPurchaseRequests.remove(identifier.split(":-:")[0]);
