...
class Billing {
  ...
  final Map<String, String> _purchasedProducts = new Map();
  ...
  Future<Map<String, String>> getPurchases() {
    if (_purchasesFetched) {
      return new Future.value(new Map.from(_purchasedProducts));
    }
    return synchronized(this, () async {
      try {
        final List purchases = await _channel.invokeMethod('fetchPurchases');
        purchases.cast().forEach((item) {
          List<String> items = item.split(":-:");
          _purchasedProducts[items[0]] = items.length > 1 ? items[1] : "";
        });
  ...
  /// Consume a product.
  Future<bool> consumeProduct(String identifier, String purchaseToken) {
    assert(purchaseToken != null);
    return synchronized(this, () async {
      try {
        final bool success = await _channel.invokeMethod('consumeProduct', {'purchaseToken': purchaseToken});
        _purchasedProducts.remove(identifier);
        return success;
      } catch (e) {
        if (_onError != null) _onError(e);
        return false;
  ...
  Future<String> purchase(String identifier) {
    assert(identifier != null);
    if (_purchasedProducts.containsKey(identifier)) {
      return new Future.value(_purchasedProducts[identifier]);
    }
    return synchronized(this, () async {
      try {
