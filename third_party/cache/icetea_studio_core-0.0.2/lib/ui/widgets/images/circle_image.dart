...
/// Animation background for onboarding screen
/// Maybe modify style if you want in there
class CircleImage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final width = MediaQuery
        ...
        .width;
    List<Widget> widgets = [];
    List<Widget> smallCircles = List(10).map((_) =>
        _smallCircle(MediaQuery
            .of(context)
            .size)).toList();
    List<Widget> largeCircles = List(5).map((_) =>
        _largeCircle(MediaQuery
            .of(context)
            .size)).toList();
