    ...
    switch (call.method) {
      case "start":
        ...
        File folder = new File(Environment.getExternalStorageDirectory() + 
                             File.separator + "RecordFire");
        // boolean xsuccess = true;
        if (!folder.exists()) {
            folder.mkdirs();
        }
        String path = call.argument("path");
        // String path = folder + xpath;
        mExtension = call.argument("extension");
        startTime = Calendar.getInstance().getTime();
        if (path != null) {
          // mFilePath = path;
          String fileName = path + "-" + String.valueOf(startTime.getTime());
          mFilePath = Environment.getExternalStorageDirectory() + File.separator + "RecordFire/" + fileName + mExtension;
        } else {
          String fileName = "record-" + String.valueOf(startTime.getTime());
          mFilePath = Environment.getExternalStorageDirectory() + File.separator + "RecordFire/" + fileName + mExtension;
        }
